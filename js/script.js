const translations = {
    en: {
        ...
        "sub-slogan": "Imagination to reality - all in one place",
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    wechatIcon.addEventListener('click', function(e) {
        ...
        if (!qrCode) {
            ...
            qrCode.id = 'wechat-qr';
            qrCode.innerHTML = '<img src="images/wechat-qr.png" alt="WeChat QR Code">';
            document.body.appendChild(qrCode);
        }

        const iconRect = wechatIcon.getBoundingClientRect();
        qrCode.style.display = 'block';
        qrCode.style.top = `${iconRect.bottom + window.scrollY + 10}px`;
        qrCode.style.left = `${iconRect.left + window.scrollX - 100}px`;
    });

    document.addEventListener('click', function(e) {
        if (qrCode && e.target !== wechatIcon && !qrCode.contains(e.target)) {
